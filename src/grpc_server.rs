...
use services::{
    ...
    ChatMessage, PaymentRequest, PaymentResponse,
...
impl ChatService for MyChatService {
    ...
    ) -> Result<Response<ReceiverStream<Result<ChatMessage, Status>>>, Status> {
        ...
        tokio::spawn(async move {
            while let Some(message) = stream.message().await.unwrap_or_else(|_| None) {
                ...
                let reply = ChatMessage {
                    ...
                    message: format!(
                        "Terima kasih telah melakukan chat kepada CS virtual, pesan anda akan dibalas pada jam kerja. pesan anda adalah : {}",
                        message.message
                    ),
                };
